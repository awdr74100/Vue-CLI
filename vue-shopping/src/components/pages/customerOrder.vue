      ...
      <form class="col-md-6" @submit.prevent="createOrder">
        ...
        <div class="form-group">
          ...
          <ValidationProvider rules="required" v-slot="{ errors }" name="name">
            <input type="text" class="form-control" :class="{'is-invalid':errors[0]}" name="name" id="username"
  ...
  export default {
    ...
    methods: {
      getProducts(page = 1) {
        ...
        const vm = this;
        const api = `${server}/api/${user}/products?page=${page}`;
        vm.isLoading = true;
      ...
      getProduct(id) {
        ...
        const user = process.env.CUSTOM_PATH;
        const vm = this;
        const api = `${server}/api/${user}/product/${id}`;
      ...
      addtoCart(id, qty = 1) {
        ...
        const user = process.env.CUSTOM_PATH;
        const vm = this;
        const api = `${server}/api/${user}/cart`;
        const cart = {
          product_id: id,
          qty: qty,
        };
      ...
      removeCartItem(id) {
        ...
        const user = process.env.CUSTOM_PATH;
        const vm = this;
        const api = `${server}/api/${user}/cart/${id}`;
        vm.isLoading = true;
      ...
      addCouponCode() {
        ...
        const api = `${server}/api/${user}/coupon`;
        vm.isLoading = true;
        this.$http.post(api, {
          data: coupon
        }).then((response) => {
      ...
      createOrder() {
        ...
        const vm = this;
        const order = vm.form;
        const api = `${server}/api/${user}/order`;
        vm.isLoading = true;
        this.$http.post(api, {
          data: order
        }).then((response) => {
